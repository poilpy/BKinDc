...
class Model(nn.Module):
    ...
    def forward(self, x, tr_x=None, gmtr_x1 = None, gmtr_x2 = None, gmtr_x3 = None,
        ...
        if tr_x is not None:
            ...
            # tusk(tr_heatmap)
            tr_u_x, tr_u_y, tr_covs, bbox, tr_confidence = self._mapTokpt(tr_heatmap, find_peaks=find_peaks, use_bbox=use_bbox)
        ...
        u_x, u_y, covs, _, confidence = self._mapTokpt(heatmap, find_peaks=find_peaks, use_bbox=use_bbox)   
        ...
        if gmtr_x1 is not None:  # Rotation loss
            ...
            gmtr_u_x, gmtr_u_y, gmtr_covs, _, _ = self._mapTokpt(gmtr_heatmap, find_peaks=find_peaks, use_bbox=use_bbox)
            ...
            gmtr_u_x_2, gmtr_u_y_2, gmtr_covs, _, _ = self._mapTokpt(gmtr_heatmap, find_peaks=find_peaks, use_bbox=use_bbox)
            ...
            gmtr_u_x_3, gmtr_u_y_3, gmtr_covs, _, _ = self._mapTokpt(gmtr_heatmap, find_peaks=find_peaks, use_bbox=use_bbox)
...
#         return bboxes.to(heatmap.device)
    ...
    def _mapTokpt(self, heatmap, find_peaks, use_bbox):
        # heatmap: (N, K, H, W) 
        if use_bbox:
        ...
        for ii in range(1, self.num_agents):
            ...
            bboxes = torch.cat([bboxes, bbox_masks[ii, :, :]], dim = 1)
